...
arrow::Result<std::unique_ptr<flight::FlightInfo>> MakeExampleFlightInfo(
  ...
  ARROW_ASSIGN_OR_RAISE(
      auto info,
      flight::FlightInfo::Make(*ExampleSchema(), descriptor, {endpoint}, 4, -1));
...
// Wrap the example batches in a FlightDataStream so DoGet can stream them.
arrow::Result<std::unique_ptr<flight::FlightDataStream>> MakeExampleStream() {
  ARROW_ASSIGN_OR_RAISE(auto batches, ExampleBatches());
  ARROW_ASSIGN_OR_RAISE(auto reader, arrow::RecordBatchReader::Make(batches));
  return std::unique_ptr<flight::FlightDataStream>(
      new flight::RecordBatchStream(std::move(reader)));
}
...
arrow::Future<> EchoExchange(
  ...
  return ::arrow::Loop([reader, writer, begun]() {
           return reader->Next().Then(
               [reader, writer, begun](flight::FlightStreamChunk chunk)
                 ...
                 return start.Then(
                     [writer, chunk = std::move(chunk)]() mutable
                         -> arrow::Future<arrow::ControlFlow<>> {
                       if (chunk.data && chunk.app_metadata) {
...
arrow::Future<> DrainPutReader(
  ...
  return reader->GetSchema().Then([reader, writer, num_batches](const std::shared_ptr<arrow::Schema>& schema) {
    PrintIndentedBlock("server received schema", schema->ToString());
    return ::arrow::Loop([reader, num_batches]() {
             return reader->Next().Then(
                 [num_batches](flight::FlightStreamChunk chunk)
                   ...
                   if (chunk.data) {
                     ++(*num_batches);
                     PrintIndentedBlock("server received batch #" +
        ...
        .Then([writer, num_batches](const arrow::internal::Empty&) {
          auto summary =
              arrow::Buffer::FromString("received " + std::to_string(*num_batches) + " batches");
...
class ExampleAsyncFlightServer : public flight::AsyncFlightServerBase {
 public:
  ...
  arrow::Future<std::unique_ptr<flight::SchemaResult>> GetSchema(
      const flight::ServerCallContext&, const flight::FlightDescriptor& descriptor) override {
  ...
  arrow::Future<std::unique_ptr<flight::FlightDataStream>> DoGet(
  ...
  arrow::Future<> DoPutAsync(
    ...
    if (!IsExampleCommandDescriptor(reader->descriptor(), kUploadCommand)) {
      return arrow::Future<>::MakeFinished(
          arrow::Status::NotImplemented("Only the '", kUploadCommand,
  ...
  arrow::Future<> DoExchangeAsync(
    ...
    if (!IsExampleCommandDescriptor(reader->descriptor(), kEchoCommand)) {
      return arrow::Future<>::MakeFinished(
          arrow::Status::NotImplemented("Only the '", kEchoCommand,
                                        "' command is implemented"));
    }
    PrintEvent("server", "DoExchange entering echo loop");
    return EchoExchange(std::shared_ptr<flight::AsyncFlightMessageReader>(std::move(reader)),
...
arrow::Status RunClientDemo() {
  ...
  auto run_demo = [&]() -> arrow::Status {
    ...
    ARROW_ASSIGN_OR_RAISE(
        auto do_put, client->DoPut(upload_descriptor, ExampleSchema()));
    ARROW_ASSIGN_OR_RAISE(auto batches, ExampleBatches());
